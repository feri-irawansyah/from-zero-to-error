...
pub fn ModalDetail() -> impl IntoView {
    ...
    view! {
        ...
        >
            <Show
                // ✅ cek Value::Null dulu
                when=move || !modal_state.data.get().is_null()
                fallback=move || view! { <h1 class="text-center">Loading...</h1> }
            >
                {move || {
                    let data_opt = modal_state.data.get().as_object().unwrap().clone();
                    let value = data_opt.clone();
                    // bisa None
                    view! {
                        <Show
                            when=move || !value.is_empty()
                            fallback=move || view! { <h1 class="text-center">Not Found</h1> }
                        >
                            <form>
                                {data_opt
                                    .iter()
                                    .filter(|(key, _)| { *key != "tsv" && *key != "ip_address" })
                                    .map(|(key, val)| {
                                        ...
                                        view! {
                                            <div class="mb-3">
                                                <label for=key.clone() class="form-label">
                                                    {key.clone()}
                                                </label>
                                                <input
                                                    type="text"
